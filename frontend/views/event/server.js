...
socket.broadcast.to(socket.room).on('disconnect', function(){
		...
		//con.query("delete from tbl_live_chat_users where users_id='"+socket.userID+"' and chatroom="+socket.room);
...
socket.on('testfunction', function(curuserinfo) {
	con.query("delete from tbl_live_chat_users where users_id='"+curuserinfo.userID+"' and chatroom="+curuserinfo.chatRoom);
	delete usernames[socket.username];
	io.sockets.in(socket.room).emit('leftchat', socket.username, socket.username + ' has disconnected');
	//socket.leave(socket.room);
	console.log('test');
